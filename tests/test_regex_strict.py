...
def extract(question, sym_name):
    pattern = rf"\b{sym_name}\s*(?:=(?:\s*[a-zA-Z0-9_]+\s*=)*|\s+is\s+)\s*([-+]?(?:\d+[.,]\d+|\d+[.,]?|[.,]\d+)(?:\s*(?:[x×*]\s*10\^?|[eE])\s*[-+]?\d+)?)\s*([m\u03bcuµnpkMG]?)[a-zA-Z]*"
    match = re.search(pattern, question, re.IGNORECASE)
    if match:
        val_str = match.group(1)
        prefix = match.group(2)
        print(f"Matched val_str: '{val_str}'")
        # normalize
...
print("q_none:", extract("q4 = ", "q4"))
print("q_dot:", extract("q4 = .5 x 10^3 m", "q4"))
print("q_comma:", extract("q4 = 2,5 e-3 m", "q4"))
